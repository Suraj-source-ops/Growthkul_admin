...
<script>
    $(document).ready(function() {
        ...
        $dynamicTableId = "#history-table-{{ $product->id }}";
        $($dynamicTableId).DataTable({
            ...
            ajax: {
                url: "{{ route('product.history.list') }}",
                type: "POST",
                data: function(d) {
    ...
    $(document).on('click', '.view-json', function () {
        ...
        const myModal = new bootstrap.Modal(document.getElementById('jsonModal'));
        myModal.show();
    });
</script>
{{-- Change Tab Script --}}
...
<script>
    ...
    $('#size-chart-history-btn').on("click", function(e) {
        $('#history-table-{{ $product->id }}').DataTable().ajax.reload();
